...
public interface ServiceItemRepository extends JpaRepository<ServiceItem, UUID> {
    ...
    @Query(value = "SELECT * FROM service_item s WHERE "
            ...
            + "AND (:activeOnly IS NULL OR s.active = :activeOnly) " + "ORDER BY s.name ASC", nativeQuery = true)
    Page<ServiceItem> findWithFilters(@Param("keyword") String keyword,
    ...
    /**
     * Finds all active ServiceItems by their IDs in a single query. This method is
     * used to avoid
     * N+1 query problems when fetching multiple service items.
     *
    ...
    /**
     * Fuzzy search for active ServiceItems with PostgreSQL pg_trgm + unaccent
     * extensions.
     * 
     * @param serviceName Service name (fuzzy search, similarity > 0)
     * @return List of active ServiceItem entities matching the search criteria (max
     *         10 results)
     */
    @PreAuthorize("permitAll()")
    @Query(value = """
            SELECT s.* FROM service_item s
            WHERE s.active = true
                AND (:serviceName IS NULL OR similarity(immutable_unaccent(LOWER(s.name)), immutable_unaccent(LOWER(:serviceName))) > 0)
            ORDER BY
                CASE WHEN :serviceName IS NOT NULL
                     THEN similarity(immutable_unaccent(LOWER(s.name)), immutable_unaccent(LOWER(:serviceName)))
                     ELSE 1.0
                END DESC,
                s.name ASC
            LIMIT 10
            """, nativeQuery = true)
    List<ServiceItem> fuzzySearchServiceItems(@Param("serviceName") String serviceName);
}
